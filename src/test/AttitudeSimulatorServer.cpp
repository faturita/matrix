    ...
    int ii = 0;
     while ((simTimeData.simTime.GetSeconds() <= 25) && ii < 5) 
     {
        ...
        if (read_res > 0)
        {
            ...
            cout << "PropTime = " << integrationTimes[0].GetSeconds() << " s -> " << integrationTimes[1].GetSeconds() << " s" << std::endl;
            ...
            if(client_fifo_fd != -1)
            {
                cout << "sending : " << attitudeState.time << " [ " << attitudeState.q1 << ", " 
                                                        ...
                                                        << attitudeState.w1 << ", "
                                                        << attitudeState.w2 << ", "
                                                        << attitudeState.w3 << " ]" << std::endl;
                write(client_fifo_fd, &attitudeState, sizeof(attitudeState));
                close(client_fifo_fd);
            }
            else
                cout << "Could not open Client FIFO FD" << std::endl;
        }
        else
	{
            cout << "Server has nothing to read." << std::endl;
	    ++ii;
	}
    }
    cout << "Closing Attitude Server" << std::endl;
...
{
            sleep(2);
            cout << "Running Device Model" << std::endl;
            SetupSSFComm();
            for (int ii = 0; ii < 5; ++ii)
            {
                cout << "Run " << ii << ": " << std::endl;
                SSFGetAttitude();
            }
            StopSSFComm();
}
...
int main()
{
    ...
    {
        case 0 : // child
            cout << "Running Attitude Server" << std::endl;
            AttitudeServer();
            break;
