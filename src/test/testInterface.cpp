...
int main()
{
   ...
   Select(); 
    ...
    if(myOrbit)
        if (myOrbit->IsIntegrateable())
        {
            cout << "Orbit State (km, km/s): " << ~myOrbit->GetHistoryObject().GetState(requestTime).GetStateRepresentation()->GetPositionVelocity() << std::endl;
    ...
    if(myAttitude)
        if (myAttitude->IsIntegrateable())
        {
            cout << "Attitude State: (-, rad/s)" << ~myAttitude->GetHistoryObject().GetState(requestTime).GetState() << std::endl;
