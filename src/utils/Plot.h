...
static void Plot2D(const Matrix &_data)
{
    ...
    fflush(pipeVar); // pipes are buffered, so fflush buffer after you are finished
	std::cout << "Press enter to continue." << fflush;
        char dummy;
	std::cin >> dummy;
...
static void Plot3D(const Matrix &_data)
{
    ...
    fflush(pipeVar); // pipes are buffered, so fflush buffer after you are finished
        ...
        std::cout << "Enter a key and press enter to continue..." << fflush;
        char dummy;
	std::cin >> dummy;
