...
//********************************************************************************
//
std::ostream&  operator <<(std::ostream& out_stream, const CAMrange& A)
{
//
...
{   
    ...
    {
    std::cerr <<"Error : Negative Number of Elements In Index Range " << std::endl;
    CAMmvaExit();
    } 
    if(A.stride <= 0 )
    {
    std::cerr <<"Error : Non Positive Stride in Index Range " << std::endl;
    CAMmvaExit();
    }
}
//
