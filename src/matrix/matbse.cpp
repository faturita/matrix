...
//********************************************************************************
//
std::ostream& operator <<(std::ostream& out_stream, const CAMmatrixBase& A)
{
...
std::istream& operator >>(std::istream& in_stream, CAMmatrixBase& A)
{
    const long* beginPtr  = A.Structure.indexBegin.getDataPointer();
    const long* endPtr    = A.Structure.indexEnd.getDataPointer();
    const long* stridePtr = A.Structure.indexStride.getDataPointer();
...
//
   if(INFO != 0)
   {
     if(INFO <= N)
     {
        ...
        std::cerr << " LU Factorization Stopped at Step " << INFO << std::endl;
        std::cerr << "        No Solution Returned " << std::endl;
     }
        
     if(INFO ==(N +1))
     {
        std::cerr << " Matrix Singular or Badly Conditioned " << std::endl;
        std::cerr << " Computed Solution May Be Inaccurate "  << std::endl;
        std::cerr << " Condition Number = " << 1.0/RCOND << std::endl;
     }
    }
...
void  CAMmatrixBase::indexCheck(const CAMstructureBase &S, long i1, long i2)
{
    ...
    {
     std::cerr << " Error : Incorrect Matrix Array Index Dimension " << std::endl;
...
{
    std::cerr << std::endl;
    std::cerr << "Matrix is not square : dimensions incompatable with matrix solve " << std::endl << std::endl;
    CAMmvaExit();
}
